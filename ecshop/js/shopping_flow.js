...
function checkConsignee(frm)
{
  ...
  {alert(country_not_null); return false;}

  //省份
  if (frm.elements['province'] && frm.elements['province'].value == 0 && frm.elements['province'].length > 1)
  {alert(province_not_null); return false;}

  //城市
  if (frm.elements['city'] && frm.elements['city'].value == 0 && frm.elements['city'].length > 1)
  {alert(city_not_null); return false;}

  //区域
  if (frm.elements['district'] && frm.elements['district'].length > 1 && frm.elements['district'].value == 0)
  {alert(district_not_null); return false;}

  //联系人姓名
  if (Utils.isEmpty(frm.elements['consignee'].value))
  {alert(consignee_not_null); return false}
  else if(frm.elements['consignee'].value.length < 2)
  {alert('请填写有效的联系人姓名');return false;}
  else if(frm.elements['consignee'].value.length > 18)
  {alert('联系人姓名不能大于18位');return false;}

  //证件号码
  var carAgeReg  = /^(\d{15}$|^\d{6}(19|2[0-9])\d{9}(\d|X|x))$/;
  if(Utils.isEmpty(frm.elements['card_num'].value))
  {alert(cardnum_not_null);return false;}
  else if(!Utils.isCardNum(frm.elements['card_num'].value))
  {alert(invalid_carnum);return false;}
  else if(!carAgeReg.test(frm.elements['card_num'].value))
  {alert('证件号码出生日期有误！');return false;}

  //详细地址
  if (frm.elements['address'] && Utils.isEmpty(frm.elements['address'].value))
  {alert(address_not_null);return false;}

  //手机号
  if (Utils.isEmpty( frm.elements['tel'].value))
  {alert(mobile_not_null);return false;}
  else if(!Utils.isMobile(frm.elements['tel'].value))
  {alert(mobile_invaild);return false;}
  ...
  //邮编
  if (!Utils.isEmpty(frm.elements['zipcode'].value) && !Utils.isNumber(frm.elements['zipcode'].value))
  {alert(zip_not_num);return false;}
  else if(!Utils.isEmpty(frm.elements['zipcode'].value) && frm.elements['zipcode'].value.length != 6)
  {alert('请填写6位有效邮政编码');return false;}
