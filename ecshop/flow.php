...
require(dirname(__FILE__) . '/includes/lib_form.php');
require(ROOT_PATH . 'includes/lib_order.php');

/* 载入语言文件 */
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/user.php');
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/shopping_flow.php');
require_once(__DIR__.'/includes/smsSend.php');
/*------------------------------------------------------ */
//-- INPUT
/*------------------------------------------------------ */
...
elseif ($_REQUEST['step'] == 'notify')
{
    ...
    $goods_attr=$json->decode($goods_attr,true);
	$_SESSION['start_age'] = '';
	$_SESSION['end_age'] = '';
    if(!empty($goods_attr['spec_224'])){
		preg_match('/(\d+)-(\d+)*/',$goods_attr['spec_224'],$age);
        $_SESSION['start_age'] = $age[1];
        $_SESSION['end_age'] = $age[2];
    }

    if($_POST['goods_id']){
        $goods_id = $_POST['goods_id'];
        $_SESSION['goods_id']=$goods_id;
    }else if($_SESSION['goods_id']){
    ...
    $_SESSION['policy_information']=$goods_attr;
    if(empty($goods_id)){
        $smarty->assign('errormsg','未选中商品');
        $_REQUEST['step']='problem';
        $_SESSION['goods_id']=false;
        $_SESSION['all_information']=false;
        $_SESSION['policy_information']=false;
        $_SESSION['order_id']=false;
    }
    $good=$db->getRow('select * from '.$ecs->table('goods').'where goods_id='.$goods_id);
    //投保人关系
    $relation=$good['relation'];
    /*
    $test="woshitest";
    $sql = "SELECT notify_detail from  " . $GLOBALS['ecs']->table('goods_detail') ."where goods_id=".$goods_id;
    $res = $GLOBALS['db']->getRow($sql);
    $title=$res['notify_detail'];
    //变量替换
    $match = array();
    ...
    */
    $sql = "SELECT * from  " . $GLOBALS['ecs']->table('goods_jkgz') ." where goods_id=".$goods_id." order by jkgz_id";
    $res = $GLOBALS['db']->getAll($sql);
    ...
    if(count($res)==0){
        ...
        $_REQUEST['step']='fill_policy';
       
    }else{
        $res['notify_detail']=$title;
        $smarty->assign('goods_jkgz_num',count($res));
        $smarty->assign('goods_jkgz',$res);
    }
    error_log($_SESSION['goods_id']);
    if($_SESSION['goods_id']==7){
        $smarty->assign('car_policy' ,'true');
    }else{
        $smarty->assign('car_policy' ,'false');
    }
...
elseif ($_REQUEST['step'] == 'act_problem'){
    $_SESSION['goods_id']=false;
    $_SESSION['all_information']=false;
    $_SESSION['policy_information']=false;
    $_SESSION['order_id']=false;
...
elseif ($_REQUEST['step'] == 'fill_policy'){
    $Form = new class_post();
    //验证
    $result = $Form->checkParam();
    if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
    }
    include_once('includes/lib_transaction.php');
    $policy_information=$_SESSION['policy_information'];
    if(!$policy_information){
        $smarty->assign('errormsg','商品信息失效');
        $_REQUEST['step']='problem';
        $_SESSION['goods_id']=false;
        $_SESSION['all_information']=false;
        $_SESSION['policy_information']=false;
...
elseif($_REQUEST['step'] == 'verify_policy')
{
    if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
    }
    ...
    if(!$policy_information){
        ...
        $_SESSION['policy_information']=false;
        $_SESSION['order_id']=false;
		show_message('商品信息失效,请重新下单',['返回首页'], ['/']);
    }
    //属性获取 
    $all_information = $_POST;
    //被保人年龄
    $recognizee_birth = substr($all_information['recognizee_cardnum'],6,8);
    $recognizee_age = birthday($recognizee_birth);
    if(!empty($_SESSION['start_age']) && !empty($_SESSION['end_age']) && ($recognizee_age > $_SESSION['end_age'] || $recognizee_age < $_SESSION['start_age'])){
        show_message('被保人年龄与所选年龄不符请重新选择!');
    }
    $ageArr = $db->getRow("select start_age, end_age from ". $ecs->table('goods'). "where goods_id= $_SESSION[goods_id]");
    if ((!empty($ageArr['start_age']) || !empty($ageArr['end_age'])) && ($recognizee_age > $ageArr['end_age'] || $recognizee_age < $ageArr['start_age'])){
		show_message('被保人年龄不符条件!');
    }
    $all_information['recognizee_age']=$recognizee_age;
    //性别
    $all_information['recognizee_sex']=getSex($all_information['recognizee_sex']);
    $all_information['policy_sex']=getSex($all_information['policy_sex']);
    //保险期限
    $now= strtotime("+1 day");
    if(!empty($policy_information['spec_227'])){
        switch ($policy_information['spec_227']){
            case '1-3天':
				$limit= strtotime("+3 days");
				break;
            case '4-7天':
    ...
    }else if($policy_information['spec_220']=='3个月'){
        $limit= strtotime("+3 months");
    }else{
		$limit= strtotime("+1 year");
    }
    $all_information['start_time']=$now;
    $all_information['end_time']=$limit;
	$smarty->assign('start_time',date('Y年m月d日'.'00时', $now));
	$smarty->assign('end_time',date('Y年m月d日'.'24时', $limit));
    //保单信息
    if($policy_information['combo_id']){
        $recognizee_message=getComboByGoodsId($_SESSION['goods_id'],$policy_information['combo_id'])[0][content];
        //$recognizee_message=json_encode($recognizee_message);
        
    }else{
        $recognizee_message=array();
        $recognizee_message[0]=array('key'=>'个人账户资金损失保险','value'=>$policy_information['spec_215']);
        $recognizee_message[1]=array('key'=>'账户挂失补办费用保险','value'=>$policy_information['spec_218']);
        //error_log(print_r($recognizee_message,1));
    }
    error_log(print_r($recognizee_message,1));
    $smarty->assign('recognizee_message',$recognizee_message);
    $all_information['$recognizee_message']=json_encode($recognizee_message);
    $goods_id=$_SESSION['goods_id'];
    $combo_id=$policy_information;
    $combo_id['birthday'] = $recognizee_birth;
    if(file_exists(__DIR__."/api/functions/index.php")){
        include "./api/functions/index.php";
        $policy_money = calculatePrice($combo_id, $goods_id);
        $smarty->assign('policy_money',$policy_money);
    }

    $good_sm = $db->getOne('SELECT brand_desc FROM ' . $ecs->table('goods') . ' goods ,'.$ecs->table('brand').' brand  WHERE goods.brand_id=brand.brand_id and goods.goods_id='.$goods_id);
    ...
    $relation=$good['relation'];
    if($relation == 1){
        $all_information['recognizee_relation']="监护人";
    }else{
        $all_information['recognizee_relation']="本人";
    }
    
    $_SESSION['all_information']=$all_information;
    //商品信息
    $smarty->assign('good',$good);
    //声明
    $smarty->assign('good_sm',$good_sm);
    //保险信息
    $smarty->assign('all_information',$all_information);
   
}
elseif($_REQUEST['step'] == 'verify_car_policy')
{
    if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
    }

    $sms_code = $_POST['recognizee_phone_check'];
    $mobile = $_POST['recognizee_phone'];

    if (!verifySmsCode($mobile, $sms_code)){
        show_message('验证码不正确!');
    }else{
		//属性获取
		$all_information= $_POST;
		...
		$goods_id=$_SESSION['goods_id']?$_SESSION['goods_id']:7;
        ...
        //保险信息
        $smarty->assign('all_information',$all_information);
        $_REQUEST['step']='pay_car_policy';
    }
    
}
elseif($_REQUEST['step'] == 'pay_policy'){
   if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
   }
   if(empty($_SESSION['policy_information'])){
	   show_message('商品信息失效',['返回首页','查看订单'], ['/','user.php']);
   }
   $policy_information=$_SESSION['policy_information'];
   $all_information=$_SESSION['all_information'];
   unset($all_information['toJSONString']);
   if(!$policy_information){
       $smarty->assign('errormsg','商品信息失效');
       $_REQUEST['step']='problem';
       $_SESSION['goods_id']=false;
   ...
   $goods_id=$_SESSION['goods_id'];
   $combo_id=$policy_information;
   $combo_id['birthday'] = substr($all_information['recognizee_cardnum'],6,8);
   ...
   $good=$db->getRow('select * from '.$ecs->table('goods').'where goods_id='.$goods_id);
   $smarty->assign('policy_information',$policy_information);
   $smarty->assign('good',$good);
   // 给货到付款的手续费加<span id>，以便改变配送的时候动态显示
   $payment_list = available_payment_list(1, 0);
   $total = $db->getRow('SELECT user_money FROM ' . $ecs->table('users') . '  WHERE user_id='.$_SESSION['user_id']);
   $smarty->assign('user_money',$total['user_money']);
   $smarty->assign('payment_list', $payment_list);
   
//   var_dump($policy_money);
//   exit;
   
   $good_price=floatval($total['user_money'])-floatval($policy_money);
   /* 插入订单商品 */
   $order = array();
   $order['order_sn'] = get_order_sn(); //获取新订单号
   $order['user_id'] = $_SESSION['user_id'];
   $order['card_num'] = $_SESSION['all_information']['recognizee_cardnum'];
   $order['consignee'] = $_SESSION['all_information']['recognizee_name'];
   $order['mobile'] = $_SESSION['all_information']['recognizee_phone'];
   $order['order_status']=0;
   $order['pay_status']=0;
   $order['pay_id']=1;
   $order['pay_name']='<font color="#FF0000">余额付款</font>';
   $order['pay_fee']=$policy_money;
   $order['order_amount']=$policy_money;
   ...
   $order_good['goods_id']=$goods_id;
   $order_good['goods_name']=$good['goods_name'];
   $order_good['goods_sn']=$good['goods_sn'];
   $order_good['goods_price']=floatval($policy_money);
   $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_goods'), $order_good, 'INSERT');
   /*
   $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
       "order_id, goods_id, goods_name, goods_sn,  ".
       "goods_price) values (".$new_order_id.",".$goods_id.",'".$good['goods_name']."','".$good['goods_sn']."','".floatval($policy_money)."')";
   $db->query($sql);
   */
   //插入保险单
   $policy = array();
   $policy['order_id']=$new_order_id;
   $policy['user_id']=$_SESSION['user_id'];
   $policy['goods_id']=$goods_id;
   $policy=array_merge($policy,$all_information);
   error_log(print_r($policy,1));
   $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_policy'), $policy, 'INSERT');
   /*
   $sql = "insert into ".$ecs->table('user_policy')."(order_id,user_id,goods_id, ".
       "recognizee_name,recognizee_cardtype,recognizee_cardnum,recognizee_brithday,recognizee_sex,recognizee_phone, recognizee_age,recognizee_timehorizon,recognizee_relation".
       "policy_name,policy_cardtype,policy_cardnum,policy_brithday,policy_sex,policy_phone) values ".
       "(".$new_order_id.",".$_SESSION['user_id'].",".$goods_id.",'".$all_information['recognizee_name']."',".$all_information['recognizee_cardtype'].
       ",'".$all_information['recognizee_cardnum']."','".$all_information['recognizee_brithday']."',".$all_information['recognizee_sex'].",".$all_information['recognizee_phone'].
       ",'".$all_information['recognizee_age']."','".$all_information['recognizee_timehorizon']."','".$all_information['recognizee_relation'].
       "',".$all_information['policy_name']."',".$all_information['policy_cardtype'].
       ",'".$all_information['policy_cardnum']."','".$all_information['policy_brithday']."',".$all_information['policy_sex'].",".$all_information['policy_phone'].")";
   $db->query($sql);
   */
   unset($_SESSION['policy_information']);
}elseif($_REQUEST['step'] == 'user_charge'){
   if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
   }
   $pay_money=floatval($_POST['money']);
   if(!is_numeric($pay_money) || $pay_money <= 0){
       show_message('充值金额不正确!');
   }
   /* 插入订单商品 */
   $order = array();
   $order['order_sn'] = get_order_sn(); //获取新订单号
   $order['user_id'] = $_SESSION['user_id'];
   $order['consignee'] = '用户充值';
   $order['order_status']=0;
   $order['pay_status']=0;
   $order['pay_id']=5;
   $order['pay_name']='在线充值';
   $order['pay_fee']=$pay_money;
   $order['order_amount']=$pay_money;
   $order['add_time']=gmtime();
   $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $order, 'INSERT');
   //插入订单
   $order['order_id']  = $db->insert_id();
   $order['goods_name'] = $order['pay_name'];

	$conf = $db->getRow("select config from". $ecs->table('config')." where code = 'jhf.pay'");
	$config = json_decode($conf['config'], true, JSON_UNESCAPED_UNICODE);
	require_once __DIR__.'/includes/jhpay/Jhpay.php';
	require_once __DIR__.'/includes/jhpay/JhpaySubmit.php';

	//构造参数
	$Jhpay = new Jhpay($config);
	$html = $Jhpay->get_code($order);
	echo $html;
}
elseif($_REQUEST['step'] == 'act_car_pay_policy'){
    if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
    }elseif (empty($_SESSION['goods_id'])){
        show_message('商品信息失效','返回首页', '/');
    }
    $all_information=$_SESSION['all_information'];
    unset($all_information['toJSONString']);
    if(!$all_information){
    ...
    $order_good['goods_price']=floatval($policy_money);
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_goods'), $order_good, 'INSERT');
    //error_log(print_r($order_good,1));
    //插入保险单
    $policy = array();
    $policy['order_id']=$new_order_id;
    $policy['user_id']=$_SESSION['user_id'];
    $policy['goods_id']=$goods_id;
    $policy['start_time'] = strtotime("+1 day");
    $policy['end_time']   = strtotime("+1 year");
    $policy=array_merge($policy,$all_information);
    $policy['recognizee_age'] = getAgeByID($policy['recognizee_cardnum']);
    $policy['recognizee_brithday'] = substr($policy['recognizee_cardnum'],6,4).'-'.substr($policy['recognizee_cardnum'],10,2).'-'.substr($policy['recognizee_cardnum'],12,2);
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_car_policy'), $policy, 'INSERT');
    $_SESSION['goods_id']=false;
    $_SESSION['all_information']=false;
    $_SESSION['policy_information']=false;
    show_message('订单提交成功', ['查看订单','回到首页'], ['user.php', '/']);

}
elseif ($_REQUEST['step'] == 'act_alipay_policy'){
	if(!$_SESSION['user_id']){
		ecs_header("Location: ./user.php\n");
	}
	//alipay
	...
	$config['return_url'] = ALIPAY_RETURN_URL;
	$config['notify_url'] = ALIPAY_NOTIFY_URL;
//	$config['gatewayUrl'] = 'https://openapi.alipaydev.com/gateway.do';
	//订单
	$order = $db->getRow('SELECT o.order_amount, o.order_sn, g.goods_name FROM ' . $ecs->table('order_info') .'o left join '.$ecs->table('order_goods') .'g on o.order_id=g.order_id'. '  WHERE o.user_id='.$_SESSION['user_id'].' and o.order_id=' .$_SESSION['order_id']. ' and pay_status = 0');
	if(!$order){
	    show_message('订单获取失败!');
	}
	require_once __DIR__.'/includes/alipay/pagepay/service/AlipayTradeService.php';
	require_once __DIR__.'/includes/alipay/pagepay/buildermodel/AlipayTradePagePayContentBuilder.php';
...
elseif($_REQUEST['step'] == 'act_pay_policy'){
    if(!$_SESSION['user_id']){
        ecs_header("Location: ./user.php\n");
    }
    $policy_information=$_SESSION['policy_information'];
    ...
    if(empty($policy_information)){
        $total = $db->getRow('SELECT user_money FROM ' . $ecs->table('users') . '  WHERE user_id='.$_SESSION['user_id']);
        $user_money=$total['user_money'];
        $order_id=$_SESSION['order_id'];
        $order = $db->getRow('select pay_fee,pay_status from '.$ecs->table('order_info').' where order_id='.$order_id);
        $policy_money=$order['pay_fee'];
        $pay_status=$order['pay_status'];
        if($pay_status == PS_PAYED){
            $smarty->assign('errormsg','商品已经付款');
            $_REQUEST['step']='problem';
        }else{
            if(floatval($policy_money)>floatval($user_money)){
                $smarty->assign('errormsg','商品金额大于余额');
                $_REQUEST['step']='problem';
            }else{  
                $good_price=floatval($user_money)-floatval($policy_money);
                /* 插入订单商品 */
                $sql = "UPDATE " .$GLOBALS['ecs']->table('users')." SET user_money = ".$good_price." WHERE user_id=".$_SESSION['user_id'];
                $db->query($sql);
                ...
                $order_id=$_SESSION['order_id'];
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $order, 'UPDATE',"order_id='$order_id'");
                $_SESSION['goods_id']=false;
                $_SESSION['all_information']=false;
                $_SESSION['policy_information']=false;
//                ecs_header("Location: ./user.php\n");
        ...
        if(floatval($policy_money)>floatval($user_money)){
            ...
            $_SESSION['policy_information']=false;
//            ecs_header("Location: ./user.php\n");
			show_message('订单支付成功','查看订单','/user.php');
        }
    }
}
else
...
function getComboByGoodsId($goods_id,$combo_id){
    $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('combo') ." where gid = ".$goods_id." and id=".$combo_id." order by sort_order asc";
    $combo_res = $GLOBALS['db']->getAll($sql);
    foreach ($combo_res as $k => $v){
        $combo_res[$k]['content'] = json_decode($v['content'],true);
    }
    return $combo_res;
}
?>
