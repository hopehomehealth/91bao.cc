...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if (empty($is_url_goods_img))
            {
                ...
                {
                    $goods_img = $image->make_thumb('../'. $goods_img , 350,350);
                    if ($goods_img === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
    ...
    $goods_short_name = !empty($_POST['goods_short_name']) ? $_POST['goods_short_name'] :$_POST['goods_name'];
	...
	$start_age = $_POST['start_age']?intval($_POST['start_age']) : '0';
	$end_age = $_POST['end_age']?intval($_POST['end_age']) : '100';;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_short_name, start_age, end_age, goods_name_style, goods_sn, " .
            "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_short_name', '$start_age', '$end_age', '$goods_name_style', '$goods_sn', '$catgory_id', " .
...
elseif ($_REQUEST['act'] == 'edit_virtual_sales')
{
	...
	$id = $_POST['id'];
	if(file_exists("./api/functions/{$id}.php")){
