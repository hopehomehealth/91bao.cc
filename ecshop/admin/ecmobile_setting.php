...
if ($_REQUEST['act']== 'list')
{
    ...
    $config = json_encode($_POST['value'], JSON_UNESCAPED_UNICODE);
    $status = $_POST['value']['status'];
    $time = date('Y-m-d H:i:s',time());

    if($res){
        $sql = "UPDATE ".$ecs->table('config')." SET `updated_at` = '$time',`status` = '$status' ,`config` = '".json_encode($_POST['value'], JSON_UNESCAPED_UNICODE)."' WHERE `code` = '$code'";
    }else{
                ...
                0 => array(
                    ...
                    'vars' => array(
                        ...
                        3 => array(
                            'type' => 'text',
                            'name' => '支付宝公钥',
                            'code' => 'alipay_public_key',
                            'value' => '',
                        ),
                        4 => array(
                            'type' => 'text',
                            'name' => '商户私钥',
                            'code' => 'merchant_private_key',
                            'value' => '',
                        ),
                        5 => array(
                            'type' => '签名格式',
