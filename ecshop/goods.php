...
//    $attr_id    = isset($_REQUEST['attr']) ? explode(',', $_REQUEST['attr']) : array();
    ...
    else
    {
        ...
        //商品价格计算
        if(!empty($_REQUEST['type']) && $_REQUEST['type'] == "static"){
            $shop_price = $_REQUEST['price'];
        }else if(!empty($_REQUEST['type']) && $_REQUEST['type'] == "dynamic"){
			include __DIR__."/api/functions/{$goods_id}.php";
			$req_arr = array_map(mysql_real_escape_string(),$_REQUEST);
			$shop_price =calculatePrice($req_arr,$goods_id);
        }else{
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'gotopage')
{
    ...
    if (!empty($goods_id))
    {
        ...
        foreach ($bought_notes as $key => $val)
        {
            ...
            $bought_notes[$key]['user_name'] = substr_replace($bought_notes[$key]['user_name'],'****',4,4);
        }
...
if (!$smarty->is_cached('goods.dwt', $cache_id) && !$smarty->is_cached('goods_insurance.dwt', $cache_id) && !$smarty->is_cached('goods_car.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('goods_total',        get_total_count($goods_id));
        ...
        $smarty->assign('combo',              getComboByGoodsId($goods_id));                     // 商品套餐
...
if($goods[cat_id] == "8"){
    $smarty->display('goods_car.dwt',      $cache_id);
}else{
    $smarty->display('goods_insurance.dwt',      $cache_id);
}
...
function getComboByGoodsId($goods_id){
	$sql = "SELECT * FROM ". $GLOBALS['ecs']->table('combo') ." where gid = ".$goods_id." and is_show =1 order by sort_order asc";
...
function get_total_count($goods_id){
	/* 查询该商品销量 */
	$sql = 'SELECT SUM(g.goods_number) ' .
		'FROM ' .
		$GLOBALS['ecs']->table('order_goods') . ' AS g ' .
