...
$not_login_arr =
array('sms_get_password','ajax_validate_sms','ajax_validate_sms_phone_login','ajax_islive','ajax_validate_vcode','login','act_login','phone_login','act_phone_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('sms_get_password','register', 'login','phone_login', 'profile', 'order_list','car_order_list', 'order_detail', 'car_order_detail','address_list', 'collection_list',
'message_list', 'tag_list', 'balance_recharge', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log','order_detail', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','delivery_info');
...
if ($action == 'default')
{
    ...
    /*获取最新3条订单 @author 姜涂 2017-06-21*/
    require('admin/includes/lib_main.php');
    $sql = "select policy.order_id,goods.goods_name,policy.recognizee_name,goods.goods_price,info.order_sn,policy.start_time,policy.end_time,info.add_time,info.pay_status,info.order_status".
    " from ".$ecs->table('order_info')." info, ".$ecs->table('order_goods')." goods, ".$ecs->table('user_policy') ."policy ".
    "where goods.goods_id = policy.goods_id AND info.pay_status <> 9 AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.user_id = ".$_SESSION['user_id']." order by info.add_time desc limit 3";

    $car_sql = "select policy.car_brand,policy.car_num,policy.frame_number,policy.order_id,goods.goods_name,policy.recognizee_phone,goods.goods_price,goods.goods_id, info.order_sn,policy.start_time,policy.end_time,info.add_time,info.pay_status".
        " from ".$ecs->table('order_info')." info, ".$ecs->table('order_goods')." goods, ".$ecs->table('user_car_policy') ."policy ".
        "where goods.goods_id = policy.goods_id AND info.pay_status <> 9 AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.user_id = ".$_SESSION['user_id']." order by info.add_time desc limit 3";
    $order_list=$db->getAll($sql);

    foreach ($order_list as $key => $value) {
        $order_list[$key]['type'] = 1;
        $order_list[$key]['start_time'] = date("Y年m月d日", $order_list[$key]['start_time'])."00时";
        $order_list[$key]['end_time']   = date("Y年m月d日", $order_list[$key]['end_time'])."24时";
    }
    $car_order_list=$db->getAll($car_sql);
    foreach ($car_order_list as $key => $value) {
        $car_order_list[$key]['type'] = 2;
    }
    $latest_list = array_merge($order_list, $car_order_list);
    ...
    $arrSort = array();
    foreach($latest_list AS $uniqid => $row){  
        foreach($row AS $key=>$value){  
            $arrSort[$key][$uniqid] = $value;  
        }  
    }
    if($sort['direction']){  
        array_multisort($arrSort[$sort['field']], constant($sort['direction']), $latest_list);  
    }
    $latest_list_3 = array_slice($latest_list, 0, 3);
    foreach ($latest_list_3 as $key => $value) {
        $latest_list_3[$key]['add_time'] = date('Y-m-d', $latest_list_3[$key]['add_time']);
    }
    /*最新3条订单结束*/
    $smarty->assign('info',        get_user_default($user_id));
    $smarty->assign('user_notice', $_CFG['user_notice']);
    $smarty->assign('prompt',      get_user_prompt($user_id));
    $smarty->assign('order_list', $latest_list_3);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (6 > strlen($password) || strlen($password) > 18)
        {
            show_message('密码必须为6到18位符!');
        }
		...
		if(verifySmsCode($username ,$sms_code)){
...
elseif ($action == 'act_phone_login')
{
    ...
    if (!preg_match("/^1[34578]{1}\d{9}$/",$username)){
		show_message('手机号格式不正确', $_LANG['relogin_lnk'], 'user.php', 'error');
    } elseif (!verifySmsCode($username, $sms_code)){
		show_message('短信验证码错误', $_LANG['relogin_lnk'], 'user.php', 'error');
	}
	if ($user->check_user($username)){
		if ($user->phone_login($username, isset($_POST['remember'])))
		{
			update_user_info();
			recalculate_price();
		...
		if (register($username, $password, $email, '手机注册') !== false)
		{
			/*把新注册用户的扩展信息插入数据库*/
			$sql = 'SELECT id FROM ' . $ecs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有自定义扩展字段的id
			$fields_arr = $db->getAll($sql);
			...
			if ($user->login($username, $password,isset($_POST['remember'])))
			{
				...
				$ucdata = isset($user->ucdata)? $user->ucdata : '';
				show_message($_LANG['login_success'] . '<br><br><br>默认密码为手机号后6位,请尽快修改' ,
					array('先去逛逛', '去修改密码'),
					array('/','user.php?act=profile'),
					'info',false);
			}
		}
		else
...
//	$confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';
    ...
    if (strlen($new_password) > 18)
    {
        show_message('密码长度不能大于18位!');
    }
...
/* 代金券充值 */
elseif ($action == 'act_add_coupon')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'includes/lib_form.php');
	$Form = new class_post();
    $telephone = $_SESSION['user_name'] ;
    $user_id = $_SESSION['user_id'] ;
    if(!$Form->fun_mobile($telephone)){
		$err->show($_LANG['back_up_page'], 'user.php?act=bonus');
    }

    $coupon_sn = mysql_real_escape_string($_POST['lname']);


    if (add_coupon($telephone,$user_id, $coupon_sn))
    {
        show_message('优惠券充值成功!',$_LANG['back_up_page'], 'user.php?act=tag_list', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'user.php?act=tag_list');
    }
}
...
elseif ($action == 'ajax_order_list')
{
    ...
    require('admin/includes/lib_main.php');
    $sql = "select goods.goods_id, info.shipping_status, policy.order_id,goods.goods_name,policy.recognizee_name,goods.goods_price,info.order_sn,policy.start_time,policy.end_time,info.add_time,info.pay_status, info.order_status,info.order_amount ".
    " from ".$ecs->table('order_info')." info, ".$ecs->table('order_goods')." goods, ".$ecs->table('user_policy') ."policy ".
    "where goods.goods_id = policy.goods_id AND info.pay_status <> 9 AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.user_id = ".$_SESSION['user_id'] ." order by info.add_time desc";
    $order_list=$db->getAll($sql);
	foreach ($order_list as $k => $order) {
		$order_list[$k]['status'] = get_order_status($order['order_status'],$order['pay_status'], $order['shipping_status'], $order['order_id']);
    }
    make_json_result($order_list);exit;
}
elseif ($action == 'ajax_car_order_list')
{
    require('admin/includes/lib_main.php');
    $sql = "select goods.goods_id, info.shipping_status, policy.car_brand,policy.car_num,policy.frame_number,policy.order_id,goods.goods_name,policy.recognizee_phone,goods.goods_price,goods.goods_id, info.order_sn, policy.start_time, policy.end_time,info.add_time, info.pay_status, info.order_status ".
        " from ".$ecs->table('order_info')." info, ".$ecs->table('order_goods')." goods, ".$ecs->table('user_car_policy') ."policy ".
        "where goods.goods_id = policy.goods_id AND info.pay_status <> 9 AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.user_id = ".$_SESSION['user_id']." order by info.add_time desc";
    $order_list=$db->getAll($sql);
	foreach ($order_list as $k => $order) {
		$order_list[$k]['status'] = get_car_order_status($order['order_status'],$order['pay_status'], $order['shipping_status'], $order['order_id']);
	}
    make_json_result($order_list);exit;
}
elseif ($action == 'ajax_order_detail')
{
    $order_id=$_GET['order_id'];
    require('admin/includes/lib_main.php');
    $sql = "select *".
        " from ".$ecs->table('order_info')." info, ".$ecs->table('order_goods')." goods, ".$ecs->table('user_policy') ."policy ".
        "where goods.goods_id = policy.goods_id AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.order_id = ".$order_id;
    $order_detail=$db->getRow($sql);
    make_json_result($order_detail);exit;
}
elseif($action == 'order_detail')
{
    ...
    $sql = "select *".
        ...
        "where goods.goods_id = policy.goods_id AND goods.order_id = info.order_id AND info.order_id = policy.order_id AND policy.order_id = ".$order_id;
    $order_detail=$db->getRow($sql);
    $order_detail['add_time']=date("Y-m-d H:i",$order_detail['add_time']);
    $smarty->assign('order_detail',$order_detail);
    $smarty->assign('start_time', date('Y年m月d日', $order_detail['start_time']).'00时');
    $smarty->assign('end_time', date('Y年m月d日', $order_detail['end_time']).'24时');
    $smarty->assign('action','order_detail');
    $smarty->display('user_transaction.dwt');
}
elseif($action == 'car_order_detail')
{
    $order_id=$_REQUEST['order_id'];
    require('admin/includes/lib_main.php');
    $sql = "select *".
    ...
    $order_detail['add_time']=date("Y-m-d H:i",$order_detail['add_time']);
    $smarty->assign('order_detail',$order_detail);
    $smarty->assign('action','car_order_detail');
    $smarty->display('user_transaction.dwt');  
}
elseif($action == 'ajax_order_delete')
{
    require('admin/includes/lib_main.php');
    $order_id=$_GET['order_id'];
    ...
    if($pay_status == 2){
        make_json_error('不允许删除已付款订单');exit;
    }else{
        $db->query("UPDATE " .$ecs->table('order_info').
                       " SET pay_status = 9 WHERE order_id = '$order_id'");
        make_json_result('',"成功删除订单");exit;
    }
}elseif($action == 'ajax_order_cancel')
{
    require('admin/includes/lib_main.php');
    $order_id=$_GET['order_id'];
    $sql="select pay_status from ".$ecs->table('order_info')." where order_id=".$order_id;
    ...
    $pay_status=$res['pay_status'];
    if($pay_status == 2){
        make_json_error('不允许取消已付款订单');exit;
    }else{
        $db->query("UPDATE " .$ecs->table('order_info').
                       " SET order_status = 2 WHERE order_id = '$order_id'");
        make_json_result('',"成功取消订单");exit;
    }
}
elseif($action == 'ajax_car_order_delete')
{
    require('admin/includes/lib_main.php');
    $order_id=$_GET['order_id'];
    $sql="select pay_status from ".$ecs->table('order_info')." where order_id=".$order_id;
...
elseif($action == 'order_buy')
{
    ...
    $sql="select goods_id from ".$ecs->table('user_policy')." where order_id=".$order_id;
    $res=$db->getRow($sql);
    ...
    $order = $db->getRow('select pay_fee, order_amount from '.$ecs->table('order_info').' where order_id='.$order_id);
    $smarty->assign('policy_money',$order['order_amount']);
    $smarty->assign('good',$good);
    $smarty->assign("step","pay_policy");
    $smarty->assign('user_money',$total['user_money']);
    $smarty->assign('payment_list', $payment_list);
    $smarty->display('flow.dwt');
}elseif($action == 'order_buy_car')
{
	assign_template();
	assign_dynamic('flow');
	$position = assign_ur_here(0, $_LANG['shopping_flow']);
    ...
    $order_id=$_REQUEST['order_id'];
    $_SESSION['order_id']=$order_id;
    $sql="select goods_id from ".$ecs->table('user_car_policy')." where order_id=".$order_id;
    $res=$db->getRow($sql);
    $goods_id=$res['goods_id'];
    $good=$db->getRow('select * from '.$ecs->table('goods').' where goods_id='.$goods_id);
    $payment_list = available_payment_list(1, 0);
...
elseif ($action == 'act_edit_address')
{
    ...
    include_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/shopping_flow.php');
    $smarty->assign('lang', $_LANG);
    $verify = new class_post();
    if (!$verify->fun_mobile($_POST['tel'])){
		show_message('手机号不正确','返回上一级');
    } elseif (!$verify->fun_status($_POST['card_num'])){
		show_message('身份证号不正确','返回上一级');
	} elseif (!$verify->fun_birthday($_POST['birthdate'])){
		show_message('生日不正确','返回上一级');
	} elseif (!$verify->fun_email($_POST['email'])){
		show_message('邮箱地址不正确','返回上一级');
	}
    ...
    $address = array(
        ...
        'card_num'   => isset($_POST['card_num'])  ? compile_str(trim($_POST['card_num'])) : '',
        'sex'   => isset($_POST['sex'])  ? compile_str(trim($_POST['sex'])) : 0,
...
elseif ($action == 'tag_list')
{
    ...
    $smarty->assign('money', $total['user_money']);
    $smarty->display('user_clips.dwt');
}
/* 余额支付 */
...
elseif ($action == 'ajax_delivery_info') {
	...
	//找回密码和注册
	$res = json_decode(str_replace('\\','',$_POST['JSON']),1);
	$mobile = $res['mobile'];
	$is_send = sendSms($mobile,'register');
	echo json_encode($is_send);

}elseif($action == 'ajax_validate_sms_phone_login'){

	//手机号登录
	$res = json_decode(str_replace('\\','',$_POST['JSON']),1);
	$mobile = $res['mobile'];
	$is_send = sendSms($mobile,'login');
	echo json_encode($is_send);
    
}elseif ($action == 'ajax_sms_car'){
	ini_set('display_errors',1);
	//车险手机验证码
	$res = json_decode(str_replace('\\','',$_POST['JSON']),1);
	$mobile = $res['mobile'];
	$is_send = sendSms($mobile,'car');
	echo json_encode($is_send);
    ...
    {
        $sms_code = trim($_POST['sms_code']);        $username  = $_POST['username'];
        //判断短信验证码
        if(verifySmsCode($username, $sms_code)){
...
}elseif($action == 'reset_password_rep'){

}

elseif($action == 'ajax_islive'){
    require('admin/includes/lib_main.php');
    $user_id=$_SESSION['user_id'];
    make_json_result($user_id);exit;
}

function get_order_status($order_status, $pay_status, $shipping_status, $order_id){
	// 已取消
	if (in_array($order_status, ['2', '3' ]) && $pay_status != 2) {
		return 5;
	}

	// 未付款
	if (in_array($pay_status, [1, 0])) {
		return 1;
	}


	//过保
	if ($order_status == 6 && $pay_status == '2' && $shipping_status == '2') {
		return 6;
	}

	// 保障中
	if ($shipping_status == 2 && $pay_status == 2 && $order_status == 1) {
		// 判断评价状态 已完成
		$sql = "select count(1) count from 91bao_order_review WHERE order_id = '$order_id'";
	...
	// 已付款 处理中
	if ($pay_status == 2 && $order_status == 0) {
		return 2;
	}
}

function get_car_order_status($order_status, $pay_status, $shipping_status, $order_id){
	// 已取消
	if (in_array($order_status, [2, 3]) && $pay_status != 2) {
		return 5;
	}
	...
	// 未付款
	if (in_array($pay_status, [1 , 0]) &&  $order_status == 1) {
		return 2 ;
	}
	...
	if ($order_status ==  1  && $pay_status == 2 && $shipping_status == 2) {
		// 判断评价状态 已完成
		$sql = "select count(1) from 91bao_order_review WHERE order_id = '$order_id'";
		$row = $GLOBALS['db']->getRow($sql);
		if ($row['count'] != 0 ) {
			return 4;
		}
