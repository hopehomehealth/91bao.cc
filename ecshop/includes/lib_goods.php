...
{
    $three_arr = array();
        $child_sql = 'SELECT cat_id, goods_name, goods_short_name, goods_id ' .
                'FROM ' . $GLOBALS['ecs']->table('goods') .
                "WHERE cat_id = '$tree_id' AND is_on_sale = 1 and is_delete =0 ORDER BY sort_order ASC, cat_id ASC";
        $res = $GLOBALS['db']->getAll($child_sql);
        foreach ($res AS $row)
        {
            if ($row['goods_id'])

               $three_arr[$row['goods_id']]['id']   = $row['goods_id'];
               $three_arr[$row['goods_id']]['name'] = empty($row['goods_short_name'])?$row['goods_name']:$row['goods_short_name'];
               $three_arr[$row['goods_id']]['url']  = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, c.cat_name, g.cat_id,  g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
