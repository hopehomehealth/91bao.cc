...
function add_coupon($mobile,$user_id,$coupon_sn){
	$sql = "select coupon_id , money from ".$GLOBALS['ecs']->table('coupon')."where telephone = '$mobile' and card_num = '$coupon_sn' and status = 0";
	$res = $GLOBALS['db']->getRow($sql);
	if($res['coupon_id']){
	    $sql = "update ".$GLOBALS['ecs']->table('coupon')." set
	     user_id = '$user_id',
	     status = 1
	     where coupon_id ='$res[coupon_id]'";
		$GLOBALS['db']->query($sql);
		log_account_change($user_id, $res['money'], 0, 0,0, '用户代金券充值PC', ACT_SAVING );
		return true;
	} else{
		$GLOBALS['err']->add('您输入的卡号不存在或已失效!');
		return false;
	}
