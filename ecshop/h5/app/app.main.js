(function () {
	...
	function AppController($scope, $location, $localStorage, $window, $rootScope, $state, $controller, CONSTANTS, API, ENUM, ngToast, CartModel, AppAuthenticationService,ConfigModel) {
         ...
         * @param navTitle	险种名称
         */
		$rootScope.goShoppingMall = function (category,sortkey,sortValue) {
			$state.go('search-result',{
                sortKey: (!sortkey) ? ENUM.SORT_KEY.DEFAULT : sortkey,
                sortValue: (!sortValue) ? ENUM.SORT_VALUE.DEFAULT : sortValue,
                keyword: null,
		...
		//客服
		$rootScope.goCustomerService = function () {
			$state.go('customer-service',{

			});
        	};

		//首页-车险订单
		$rootScope.goCarOrder = function () {
			$state.go('car-order',{
				tab: 'all'
			});
        	};
