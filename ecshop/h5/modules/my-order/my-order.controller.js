(function () {
	...
	function MyOrderController($scope, $http, $window, $timeout, $location, $state, $stateParams, API, ENUM, PaymentModel, MyOrderModel, OrderExpressModel) {
		...
		$scope.TAB_ALL = 0;					//全部
		$scope.TAB_CREATED = 1;				//待支付
		$scope.TAB_PAID = 2;				//保障中
		$scope.TAB_DELIVERING = 3;			//待续保
		$scope.TAB_DELIVERIED = 4;			//待评价
		...
		//Tab 转换
		$scope.currentType	= "ORDER";
		$scope.touchOrderType = _touchOrderType;
		...
		if ($stateParams.tab == 'all') {
			...
			$scope.myOrderModel.status = ENUM.ORDER_STATUS.OVER_PROTECTION;
		} else {
			$scope.currentTab = $scope.TAB_ALL;
			$scope.myOrderModel.status = null;
		}
		...
		// 车险转换
		function _touchOrderType(orderType) {
			$scope.currentType = orderType;
        	}
