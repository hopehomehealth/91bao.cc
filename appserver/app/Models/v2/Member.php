...
class Member extends BaseModel {
    ...
    protected $guarded = [];
    protected $appends = ['id','age','rank','gender','username','nickname','mobile','avatar','mobile_binded','joined_at','is_auth', 'is_completed' , 'total'];
    protected $visible = ['id','age','rank','gender','username','nickname','mobile','avatar','mobile_binded','joined_at','is_auth', 'is_completed', 'user_money', 'last_login','email', 'sex', 'total'];
	...
	public static function loginByMobile(array	$attributes)
	{
		extract($attributes);
		$model = self::where(['user_name' => $mobile])->first();
		if($model){
			...
			$model->save();
			$token = Token::encode(['uid' => $model->user_id]);
			return self::formatBody(['token' => $token, 'user' => $model->toArray(), 'is_exists' => true]);
		}else{
			$attributes['password'] = '';
			return self::createMemberByMobile($attributes);
		}
    }
	...
	public static function createMemberByMobileRoewe($mobile)
	{
	...
	{
		$uid = Token::authorization();
		$order = Order::where(['user_id'=> $uid, 'is_car' => 0 ])->count();

		$car_order = Order::where(['user_id'=> $uid, 'is_car' => 1 ])->count();
		return ['order' => $order, 'car_order' => $car_order];
    }
}
