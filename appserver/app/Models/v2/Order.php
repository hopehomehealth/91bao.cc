...
class Order extends BaseModel {
    ...
    protected $guarded = [];
    protected $appends = ['id', 'sn', 'total', 'payment', 'shipping', 'invoice', 'coupon', 'score','use_score', 'cashgift', 'consignee', 'status', 'created_at', 'updated_at', 'canceled_at', 'paied_at', 'shipping_at', 'finish_at','promos','duration'];
    protected $visible = ['id', 'sn', 'total', 'goods', 'payment', 'shipping', 'invoice', 'coupon', 'score', 'use_score', 'cashgift', 'consignee', 'status', 'created_at', 'updated_at', 'canceled_at', 'paied_at', 'shipping_at', 'finish_at','discount_price','promos', 'review_status', 'duration'];

    // ECM 订单状态
    const STATUS_CREATED    	 = 0; // 待付款
    const STATUS_PAID        	 = 1; // 已付款  处理中
    const STATUS_DELIVERING  	 = 2; // 保险公司勾通中
    ...
    public static function getList(array $attributes)
    {
		...
		$model = self::where([ ['user_id', '=', $uid], ['is_car', '=', '0'], ['pay_id', '!=', '5'] ]);
        ...
        if (isset($status)) {
            switch ($status) {
                ...
                case self::STATUS_PAID:
                    $model->whereIn('pay_status', [self::PS_PAYED])->where('order_status', self::OS_UNCONFIRMED );
                ...
                case self::STATUS_DELIVERIED:
                    $model->whereIn('shipping_status', [self::SS_RECEIVED])->where('order_status', self::OS_CONFIRMED)->where('pay_status', self::PS_PAYED);
                    break;
				//已完成
                case self::STATUS_OVER_PROTECTION:
					$model->whereIn('shipping_status', [self::SS_RECEIVED])->where('order_status', self::OS_SPLITING_PART)->where('pay_status', self::PS_PAYED);
                    break;
            }
        }
    ...
    public static function price(array $attributes)
    {
        ...
        $goods_id = $order_products[0]['goods_id'];
		if($val['is_shipping'] == 1){
			$total_amount = Price::goodsPrice($consignee_info['card_num'], $goods_id);
			if($total_amount == false){
			    return self::formatError(400,'被保人年龄不符合要求!');
			}
        	$order_price['total_price'] = $total_amount;
        	$order_price['product_price'] = $total_amount;
		}else{
			$order_price['total_price'] = Goods::price_format($total['amount'], false);        	$order_price['product_price'] = Goods::price_format($total['goods_price_formated'] + $total['discount_price'], false);
		}
		...
		//投保时长
		if(empty($order_products[0]['property'])){
			$duration = self::duration('1年');
			$attrValue = '';
		}else{
			$attrId = $order_products[0]['property'][0];
			$attrValue = GoodsAttr::where(['goods_attr_id' => $attrId])->first()->toArray();
			$duration = self::duration($attrValue['attr_name']);
		}
		$duration['startTime'] = date('Y年m月d日', $duration['startTime']).'00时';
		if($order_products[0]['goods_id'] == 3){
			$duration['endTime']   = date('Y年m月d日', strtotime("+3 months")).'24时';
		}else{
			$duration['endTime']   = date('Y年m月d日', $duration['endTime']).'24时';
		}
		$order_price['duration'] = $duration;
        //年龄条件
		$result = self::isAgeValid($attrValue, $consignee_info['card_num'], $val['start_age'], $val['end_age']);
		if($result == 'invalid age'){
			return self::formatError(400,'被保人年龄不符合要求!');
		}elseif ($result == 'invalid attr'){
			return self::formatError(400,'被保人年龄与所选年龄不一致!');
		}
	 ...
	 * @param $start_age
	 * @param $end_age
	 * @return mixed 100:年龄不符合;200:选择属性年龄不符合
	 */
    public static function isAgeValid($attr_value = '', $card_num,  $start_age, $end_age){
		$age = PolicyCar::getAgeByCard($card_num);
		$birthday = substr($card_num, 6, 8);
		$now = date('Ymd', time());
		if($now - $birthday < 30){
			return 'invalid age';
		}
		Log::info($age.$start_age.$end_age);
		if(($start_age > $age || $end_age < $age) && (!empty($start_age) || !empty($end_age))){
			return 'invalid age';
		}elseif (!empty($attr_value)){
			if(preg_match('/(\d+)-(\d+)*/', $attr_value['attr_name'], $array ) && !empty($end_age) && !empty($start_age) && ($array[1] > $age || $array[2] < $age)){
				return 'invalid attr';
			}
		}
		return 'other';
	...
	public static function duration($attr, $type =false)
	{
		...
		switch ($attr){
			...
			case '15-30天':
				...
				break;
			case '31-60天':
				$endTime =  strtotime('+60 days');
				break;
			case '61-90天':
				$endTime =  strtotime('+90 days');
				break;
			default:
    ...
    public static function cancel(array $attributes)
    {
        ...
        if ($model = self::where(['user_id' => $uid, 'order_id' => $order])->first()) {
            ...
            if ($model->pay_status != self::PS_UNPAYED && $model->is_car != 1)
            {
                return self::formatError(self::NOT_FOUND);
            }
...
//             判断订单状态
			...
			if($order_model = self::where(['order_id' => $order, 'user_id' => $uid])->first()){
				$order_model->review_status = 1;
				$order_model->save();
			}
...
//    {
//    	$order_goods = \DB::table('order_goods')->where('order_id', $this->attributes['order_id'])->select('goods_id')->first();
//        return  \DB::table('goods')->where('goods_id', $order_goods->goods_id)->select('cat_id')->get();
//    }
    ...
    {
        if($this->attributes['is_car']){
			return \DB::table('user_car_policy')->where('order_id', $this->attributes['order_id'])->select('start_time','end_time')->first();
        }else{
			return UserPolicy::where('order_id', $this->attributes['order_id'])->select('start_time','end_time')->first();
		}
    }
    ...
    public function getConsigneeAttribute()
    {
        return [
			...
			'card_num' => $this->attributes['card_num'],
			'card_num' => $this->attributes['card_num'],
        ];
    }
    ...
    {
    	if($this->attributes['is_car']){
			return self::convertOrderStatusCar(
...
//        }
		...
		//过保
		if (in_array($order_status, [self::OS_SPLITING_PART ]) && $pay_status == '2' && $shipping_status == '2') {
			return self::STATUS_OVER_PROTECTION;
		}
    ...
    private static function convertOrderStatusCar($order_id, $order_status, $pay_status, $shipping_status)
    {
        // 已取消
        if (in_array($order_status, [self::OS_CANCELED, self::OS_INVALID, self::OS_RETURNED]) && $pay_status != 2) {
            return self::STATUS_CANCELLED;
        }

		// 未付款
		if (in_array($pay_status, [self::PS_UNPAYED, self::PS_PAYING]) && $order_status == 1) {
			return self::STATUS_PAID ;
		}

        // 处理中
        if (in_array($order_status, [self::OS_UNCONFIRMED ])) {
            return self::STATUS_DELIVERING;
        }

        // 保障中

        if (in_array($order_status, [self::OS_CONFIRMED ]) && $pay_status == 2) {
            // 判断评价状态 已完成
            if (OrderReview::where('order_id', $order_id)->count()) {
                return self::STATUS_FINISHED;
            }
        ...
        //过保
		if (in_array($order_status, [self::OS_INVALID]) && $pay_status == 2) {
			return self::STATUS_OVER_PROTECTION;
		}
