...
use App\Services\Payment\Jhf\Jhpay;
...
use App\Services\Payment\AlipayWap\wappay\buildermodel\AlipayTradeWapPayContentBuilder;
use App\Services\Payment\AlipayWap\wappay\service\AlipayTradeService;
        ...
        if (isset($userAgent['Platform']) && strtolower($userAgent['Platform']) == 'wechat') {
            ...
            if ($response['result'] !== 'success')
            {
                ...
                if ($arr = self::where(['type' => 'payment', 'status' => 1, 'code' => 'cod.app'])->first()) {
                 ...
                 // 聚合富
//                if ($arr = self::where(['type' => 'payment', 'status' => 1, 'code' => 'jhf.pay'])->first()) {
//                    $arr = $arr->toArray();
//                    array_push($model, $arr);
//          		}
	...
	public static function charge(array $attributes)
	{
		extract($attributes);
		$uid = Token::authorization();

		$payment = self::where(['type' => 'payment', 'status' => 1, 'code' => 'jhf.pay'])->first();

		if (!$payment) {
			return self::formatError(self::NOT_FOUND);
		}
		$config = self::checkConfig(['app_id', 'app_key', 'app_mername'], $payment);
		...
		$order['add_time']=time();
		$order['order_id'] = \DB::table('order_info')->insertGetId($order);

		$Jhf = new Jhpay($config);
		define('IS_MOBILE', 1);
		$param = $Jhf->get_codewxsm($order);
		if($pay_type == 'wx'){
			$url = 'http://www.jhpay.com/service/wechatScanCode.jsp';
		}elseif($pay_type == 'alipay'){
			$url = 'http://www.jhpay.com/service/alipayScanCode.jsp';
		}
		$pay_type == 'wx';
		$result = curl_post($url, http_build_query($param));
    ...
    public static function pay(array $attributes)
    {
        ...
        if ($code == 'alipay.wap') {
            ...
            $config = self::checkConfig(['partner_id', 'app_id', 'merchant_private_key'], $payment);

            if (!$config) {
                return self::formatError(self::UNKNOWN_ERROR);
            }
            $config['gatewayUrl'] = 'https://openapi.alipaydev.com/gateway.do';
            $config['notify_url'] = url("/v2/order.notify.alipay.wap");
            $config['return_url'] = $_SERVER['HTTP_REFERER'];
//            $config['charset'] = 'utf-8';
//            $config['sign_type'] = 'RSA2';
//            $config['merchant_private_key'] = $config['private_key'];
//            $config['alipay_public_key'] = $config['public_key'];

			$payRequestBuilder = new AlipayTradeWapPayContentBuilder();
			$payRequestBuilder->setBody('');
			$payRequestBuilder->setSubject('91bao');
			$payRequestBuilder->setOutTradeNo('sn'.$order->order_sn);
			$payRequestBuilder->setTotalAmount( number_format($order->order_amount, 2, '.', ''));
...
//             echo $html_text;
        ...
        if ($code == 'balance') {
            ...
            if($order->order_amount < 0){
				return self::formatError(self::BAD_REQUEST, '订单金额异常');
            }
        ...
        if ($code == 'alipay.app') {
            ...
            $config = self::checkConfig(['partner_id', 'app_id', 'private_key'], $payment);
            if (!$config) {
                return self::formatError(self::UNKNOWN_ERROR);
            }
    ...
    public static function notify($code)
    {
        ...
        if ($code == 'alipay.app' || $code == 'alipay.wap') {
            ...
            $out_trade_no = ltrim($out_trade_no , 'sn');

            $order = Order::findUnpayedBySN($out_trade_no);

            $config = self::checkConfig(['partner_id', 'app_id', 'alipay_public_key'], $payment);
            if (!$config || !$order) {
                return false;
            }
//			$config['alipay_public_key'] = $config['public_key'];
			...
			$result = AlipayTradeService::check($arr, 'RSA2', $config['alipay_public_key']);
...
//                $trade_no = $_POST['trade_no'];
                ...
                if($_POST['trade_status'] == 'TRADE_FINISHED') {
                    ...
                    $order->pay_status = Order::PS_PAYED;
                    $order->pay_id = '2';
                    $order->pay_name = '聚合富手机支付';
           		    //插入付款时间
               		$order->pay_time = time();
                    $order->save();
                    OrderAction::toCreateOrUpdate($order->order_id, $order->order_status, $order->shipping_status, $order->pay_status, '聚合富手机支付');
                }
                else if ($_POST['trade_status'] == 'TRADE_SUCCESS') {
                    //修改订单状态
                    $order->pay_status = Order::PS_PAYED;
