...
class UserAddress extends BaseModel
{
    ...
    protected $appends = ['id', 'name', 'zip_code', 'regions', 'is_default', 'mobile'];

    protected $visible = ['id', 'name', 'sex', 'card_type', 'mobile', 'email', 'zip_code', 'regions', 'country','province','city', 'address', 'is_default', 'card_num','birthdate'];
    ...
    public static function get_consignee($consignee)
    {
        ...
        if ($uid > 0)
        {
            ...
            if($model = self::join('users','user_address.address_id', '=', 'users.address_id')
...
//        $model->mobile          = isset($mobile) ? $mobile : '';
        $model->tel             = isset($mobile) ? $mobile : '';
        $model->zipcode         = isset($zip_code) ? $zip_code : '';
        $model->address_name    = '';
        $model->sign_building   = '';
        $model->best_time       = '';
        $model->sex             = isset($sex) ? $sex :0;
        $model->card_num        = $card_num;
        $model->card_type       = isset($card_type)?$card_type:1;
        $model->birthdate       = substr($card_num,6,4).'-'.substr($card_num,10,2).'-'.substr($card_num,12,2);
