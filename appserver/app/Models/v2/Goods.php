...
class Goods extends BaseModel
{
    ...
    protected $visible = [
                          'id', 'category', 'brand', 'shop', 'tags', 'default_photo', 'photos','sku', 'name', 'price', 'current_price', 'discount', 'is_shipping', 'promos','stock','properties','sales_count', 'attachments','goods_desc','score','comments','good_stock','comment_count', 'is_liked', 'review_rate', 'intro_url', 'share_url', 'created_at', 'updated_at', 'combo','notice', 'is_best', 'is_new', 'is_hot', 'start_age', 'end_age'
                         ];
		...
		case self::SALE:		
		   $model->select('goods.*',DB::raw('sum('.$prefix.'order_goods.goods_number) AS total_sales'))
		    ...
		    $model->orderBy('add_time', $sort);
				...
				case self::BOUTIQUE:
		    $model->where('is_best', 1)->orderBy('add_time', $sort);
                    break;
    ...
    public static function getInfo(array $attributes)
    {
        ...
        $data = $model->with(['properties','tags','stock','attachments', 'notice'])->first();

        if (!$data) {
            return self::formatError(self::NOT_FOUND);
        }
        if (!$data->is_on_sale && $data->virtual_sales != 9999) {
	...
	// $current_price = UserRank::getMemberRankPriceByGid($product);
        ...
        $data['combo'] = Combo::getComboByGoods($product);
...
//        }
	...
	{
		return $this->hasMany('App\Models\v2\GoodNotice','goods_id','goods_id');
    }
    ...
    {
        return $this->goods_short_name?$this->goods_short_name:$this->goods_name;
    }
...
//        return GoodsGallery::getPhotosById($this->goods_id);
    ...
    public static function purchase(array $attributes)
    {
        ...
        /* 是否正在销售 */
        if ($good['is_on_sale'] == 0 && $good['virtual_sales'] != 9999) {
            return self::formatError(self::BAD_REQUEST,trans('message.good.off_sale'));
        }
		...
		//年龄条件
		$result = Order::isAgeValid('', $consignee_info['card_num'], $good['start_age'], $good['end_age']);
	...
	if ($flow_type == Cart::CART_GENERAL_GOODS && $goods_price < ShopConfig::findByCode('min_goods_amount')) // Cart::cart_amount(true, Cart::CART_GENERAL_GOODS)
        ...
        $order['card_num'] = $consignee_info->card_num;
       ...
       /* 判断是不是实体商品 */
        ...
        /* 商品价格 */
		if($good['is_shipping'] == 1){
			$order['pay_fee'] = Price::goodsPrice($consignee_info->card_num, $product);
			$order['order_amount'] = Price::goodsPrice($consignee_info->card_num, $product);
		}else{
			$order['pay_fee'] = $total['pay_fee'];
...
//        }
        ...
        /* 插入订单商品 */
        if($good['virtual_sales'] == 9999) {
			$Link = LinkGoods::getLinkGoodIds($product)->toArray();
			$goods_id = $Link[0]['link_goods_id'];
		}else{
        	$goods_id = $product;
		}
		if(empty($order_products[0]['property']) && $product != 3 ){
			$endTime = Order::duration('1年', true);
		}elseif ($product == 3){
			...
			$endTime = Order::duration($attrValue['attr_name'], true);

		}
		if(isset($is_car) != 1){
			UserPolicy::insertPolicy($new_order_id, $goods_id, $consignee, $endTime, $good['relation']);
		}
