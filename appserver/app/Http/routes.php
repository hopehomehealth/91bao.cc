...
$app->group(['namespace' => 'App\Http\Controllers\v2','prefix' => 'v2', 'middleware' => ['xss']], function($app)
{
    ...
    $app->post('ecapi.auth.signin.mobile', 'UserController@signinByMobile');

    $app->post('ecapi.auth.social', 'UserController@auth');

    $app->post('ecapi.auth.default.signup', 'UserController@signupByEmail');

    $app->post('ecapi.auth.setpassword', 'UserController@setPassword');
	...
	$app->post('ecapi.policy.car', 'RoeweController@purchaseCar');
...
$app->group(['prefix' => 'v2', 'namespace' => 'App\Http\Controllers\v2', 'middleware' => ['token', 'xss']], function($app)
{
    ...
    $app->post('ecapi.order.subtotal.car', 'CarOrderController@subtotal');

    $app->post('ecapi.payment.types.list', 'OrderController@paymentList');

    $app->post('ecapi.payment.pay', 'OrderController@pay');

    $app->post('ecapi.payment.charge', 'UserController@charge');
    ...
    $app->post('ecapi.product.purchase.car', 'GoodsController@purchaseCar');

    $app->post('ecapi.car.order.list', 'CarOrderController@getList');

    $app->post('ecapi.car.order.info', 'CarOrderController@getInfo');
